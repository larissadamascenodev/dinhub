...
const ProximosEventos = memo(({ events, onVerTodos }: Props) => (
  <div className="relative rounded-2xl overflow-hidden">
    ...
    <div
      ...
      style={{
        background: "linear-gradient(180deg, hsl(150 100% 45% / 0.08) 0%, transparent 40%, hsl(220 20% 8% / 0.2) 100%)",
      }}
    />
    ...
    >
        ...
        {[
          ...
          <span key={s.label} className="flex items-center gap-1.5 text-[10px] text-muted-foreground/60 font-medium">
            <span
              ...
              style={{
                background: `hsl(${s.accent})`,
                boxShadow: `0 0 6px hsl(${s.accent} / 0.5)`,
              }}
            />
            {s.label}
          </span>
      ...
      <div className="mx-5">
        <div
          className="h-px"
          style={{ background: "linear-gradient(90deg, transparent, hsl(220 15% 30% / 0.3), transparent)" }}
        />
      </div>
  ...
  return (
    <div className="grid grid-cols-7 gap-1 px-5 mb-3">
      {days.map((d) => (
        ...
        >
          <span className={`text-[10px] mb-0.5 ${d.isToday ? "opacity-80" : "text-muted-foreground/40"}`}>
            {d.label}
          </span>
          <span className={`text-sm font-semibold ${d.isToday ? "" : "text-muted-foreground/60"}`}>
            {d.day}
          </span>
        </div>
      ))}
