...
const ReceitasDespesasCards = memo(({ receitas, despesas, mobile }: Props) => (
  <div className={mobile ? "grid grid-cols-2 gap-3" : "grid grid-rows-2 gap-3"}>
    <button
      className={`rounded-xl border border-primary/15 bg-card hover:border-primary/30 transition-all text-left shadow-[0_2px_8px_-2px_rgba(0,0,0,0.4),inset_0_1px_0_0_rgba(255,255,255,0.06)] ${mobile ? "p-3" : "p-4"}`}
    ...
    <button
      className={`rounded-xl border border-destructive/15 bg-card hover:border-destructive/30 transition-all text-left shadow-[0_2px_8px_-2px_rgba(0,0,0,0.4),inset_0_1px_0_0_rgba(255,255,255,0.06)] ${mobile ? "p-3" : "p-4"}`}
      style={{ background: "linear-gradient(135deg, hsl(var(--destructive) / 0.06) 0%, transparent 60%)" }}
    >
      <div className="flex items-center gap-1 mb-0.5">
