...
export const DashboardOverview: React.FC = () => {
  return (
    <Section id="como-funciona">
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.05fr] gap-12 lg:gap-16 items-center min-w-0">
        <Reveal>
          ...
          <div className="mt-7 grid grid-cols-1 gap-4 lg:gap-5">
            {bullets.map((b, i) => (
              <div key={i} className="flex flex-row gap-4 items-start">
                ...
                <div>
                  <h3 className="text-white font-bold text-sm lg:text-base" style={{ fontSize: "clamp(0.9rem, 1.15vw, 1.1rem)" }}>{b.title}</h3>
                  <p className="text-[#a0a0a0] text-xs lg:text-sm mt-1 max-w-[420px]">{b.desc}</p>
                </div>
              </div>
            ))}
          </div>
