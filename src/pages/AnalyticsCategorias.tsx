...
const CategoryDetail = ({
  ...
  return (
    ...
    >
      ...
      {/* Stats + Daily Pattern → side by side on desktop */}
...
const DonutChart = ({
  ...
  const chartSize = isMobile ? 220 : 280;
  const inner = isMobile ? 60 : 80;
  const outer = isMobile ? 95 : 120;

  return (
    <GlassCard className="p-4 md:p-6">
      <p className="text-[10px] md:text-[11px] text-muted-foreground/60 font-semibold uppercase tracking-wider mb-1">
        Total de Despesas · {monthLabel}
      </p>

      <div className="relative flex items-center justify-center" style={{ height: chartSize }}>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          <p className="text-[9px] md:text-[10px] text-muted-foreground/50 uppercase tracking-wider">Total</p>
          <p className="text-xl md:text-2xl font-bold text-foreground tabular-nums">{fmt(totalExpenses)}</p>
        </div>
      </div>

      {topCategory && (
        <div className="mt-2 flex items-center justify-center gap-2 text-xs text-muted-foreground/60">
          <span className="w-2.5 h-2.5 rounded-full" style={{ background: topCategory.color }} />
          <span>
...
const CategoryList = ({
  ...
  <GlassCard className="overflow-hidden">
    ...
    <div className={`px-4 md:px-5 pb-3 space-y-1 ${!isMobile ? "max-h-[420px] overflow-y-auto" : ""}`}>
      {categoryData.map((cat, i) => {
        ...
        return (
          <motion.button
            ...
            className="w-full flex items-center gap-3 py-2.5 md:py-3 rounded-xl hover:bg-muted/20 transition-colors px-2 -mx-2"
          >
            <div
  ...
  return (
    <div className={`pb-28 ${isMobile ? "max-w-lg mx-auto" : "max-w-5xl mx-auto"}`}>
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <h1 className="text-lg md:text-xl font-bold text-foreground flex-1">Gastos por Categoria</h1>
        <MonthSelector
        ...
        {selectedCategory && selectedCatData ? (
          ...
          >
            {categoryData.length > 0 ? (
              <div className={isMobile ? "space-y-4" : "grid grid-cols-2 gap-5 items-start"}>
                <DonutChart
                  chartData={chartData}
