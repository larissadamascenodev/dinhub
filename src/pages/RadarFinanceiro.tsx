...
export default function RadarFinanceiro() {
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      {/* PROJECTIONS SECTION - Bento Style */}
      <section className="space-y-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-display font-extrabold text-white tracking-tight uppercase">Projeções de Elite</h2>
            <p className="text-[11px] text-white/20 font-bold uppercase tracking-[0.3em] mt-1">Cenários Futuros e Simulações</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2">
            <BentoCard 
              title="Cenário para o Próximo Mês" 
              ...
              <div className="relative z-10 flex flex-col justify-between h-full py-2">
                <div className="space-y-8">
                  <div className="space-y-2">
                    <p className="text-[11px] font-black text-white/30 uppercase tracking-[0.3em]">Balanço Previsto</p>
                    <div className="flex items-baseline gap-3">
                      <span className="text-5xl lg:text-6xl font-display font-black text-white tracking-tighter tabular-nums">
                        {fmt(data?.projection?.nextMonthBalance || 0)}
                      </span>
                      <div className={`flex items-center gap-1 px-3 py-1 rounded-full text-[11px] font-black uppercase tracking-widest ${ (data?.projection?.nextMonthBalance || 0) >= 0 ? 'bg-primary/10 text-primary' : 'bg-destructive/10 text-destructive'}`}>
                        {(data?.projection?.nextMonthBalance || 0) >= 0 ? <TrendingUp className="w-3 h-3" /> : <TrendingDown className="w-3 h-3" />}
                        {pct(Math.abs(data?.projection?.nextMonthBalance || 0), data?.receitas || 1)}%
                      </div>
                    </div>
                  </div>

                  <p className="text-lg text-white/60 font-medium leading-relaxed max-w-xl">
                    Baseado na sua média dos últimos 3 meses, sua rota aponta para um saldo { (data?.projection?.nextMonthBalance || 0) >= 0 ? 'positivo' : 'crítico' }. 
                    { (data?.projection?.nextMonthBalance || 0) >= 0 
                      ? " É o momento perfeito para acelerar suas metas de investimento." 
                      : " Recomendo ajustar as categorias de maior peso imediatamente." }
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-8 pt-10 border-t border-white/5">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2 text-primary/60 mb-1">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-widest">Média de Receita</span>
                    </div>
                    <p className="text-2xl font-display font-black text-white tabular-nums">{fmt(data?.projection?.avgIncome3m || 0)}</p>
                  </div>
                  <div className="space-y-1">
                    <div className="flex items-center gap-2 text-destructive/60 mb-1">
                      <ArrowDownRight className="w-4 h-4" />
                      <span className="text-[10px] font-black uppercase tracking-widest">Média de Despesa</span>
                    </div>
                    <p className="text-2xl font-display font-black text-white tabular-nums">{fmt(data?.projection?.avgExpense3m || 0)}</p>
                  </div>
                </div>
              </div>
            </BentoCard>
          ...
          {/* Quick Actions / Future Insight Card */}
            ...
            >
              <div className="space-y-8 py-2">
                <div className="p-6 rounded-3xl bg-white/[0.03] border border-white/5 space-y-4">
                  ...
                  <h4 className="text-lg font-bold text-white leading-tight">Potencial de Reserva</h4>
                  <p className="text-[13px] text-white/50 font-medium leading-relaxed">
                    Mantendo o ritmo atual, você pode acumular <span className="text-primary font-bold">{fmt((data?.projection?.nextMonthBalance || 0) * 12)}</span> em um ano.
                  </p>
                </div>
                ...
                <div className="p-6 rounded-3xl bg-white/[0.03] border border-white/5 space-y-4">
                  ...
                  <h4 className="text-lg font-bold text-white leading-tight">Blindagem Patrimonial</h4>
                  <p className="text-[13px] text-white/50 font-medium leading-relaxed">
                    Sua reserva atual cobre <span className="text-warning font-bold">{Math.round((data?.saldoAtual || 0) / (data?.projection?.avgExpense3m || 1))} meses</span> de custos fixos.
                  </p>
                </div>

                <button 
                  onClick={() => navigate("/metas")}
                  className="w-full py-4 rounded-2xl bg-white/5 border border-white/10 text-white text-[11px] font-black uppercase tracking-widest hover:bg-white/10 transition-all flex items-center justify-center gap-2"
                >
                  Otimizar Minhas Metas
