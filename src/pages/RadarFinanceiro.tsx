...
export default function RadarFinanceiro() {
  ...
  // Category diagnostics — real analysis
  const catDiagnostics = useMemo(() => {
    ...
    if (topPctVal > 30) {
      diags.push({ text: `${top.name} concentra ${topPctVal}% dos seus gastos — é aqui que mais sai dinheiro.`, variant: "danger" });
    }
    ...
    if (highImpact.length >= 3) {
      ...
      diags.push({ text: `${names} juntas consomem mais de ${pct(highImpact.reduce((s, c) => s + c.amount, 0), receitas)}% da renda. Foque nessas 3 pra ganhar margem.`, variant: "warning" });
    }

    // Positive: category decreased
    topCats.forEach(c => {
  ...
  if (loading) {
    return (
      <div className="space-y-3 pb-4">
        ...
        {[1, 2, 3].map((i) => <div key={i} className="animate-pulse h-32 rounded-[18px] bg-muted/10" />)}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3 pb-4">
      ...
      <motion.div
        ...
        style={{ background: "linear-gradient(135deg, #0f2318 0%, #0a1a0f 60%, hsl(var(--card)) 100%)", border: "1px solid rgba(74, 222, 128, 0.15)" }}
      >
        <div className="flex items-start gap-3 relative z-[1]">
          ...
          <div className="flex-1 min-w-0">
            ...
            {hubyMsg.secondary && <p className="text-[10px] text-muted-foreground/60 mt-1.5 italic">{hubyMsg.secondary}</p>}
          </div>
        </div>
      </motion.div>
                ...
                <div key={insight.id} className="rounded-[14px] bg-secondary/20 border border-border/5 p-3.5">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <span className={`text-[11px] font-bold tabular-nums ${c.text} mt-1.5 inline-block`}>Impacto: {fmt(insight.impacto_valor)}</span>
                    </div>
                  </div>
                </div>
              );
                  ...
                  return (
                    <button key={`${action.titulo}-${i}`} onClick={() => navigate(action.path)}
                      ...
                      <div className="flex flex-col items-end gap-0.5 flex-shrink-0">
                        <span className={`text-[11px] font-bold tabular-nums ${c.text}`}>{fmt(action.impacto_estimado)}</span>
          ...
          <div className="space-y-3">
            <div className="space-y-1.5">
              <div className="h-3 rounded-full bg-border/10 overflow-hidden flex">
                <motion.div initial={{ width: 0 }} animate={{ width: `${cardPct}%` }} transition={{ duration: 0.8 }} className="h-full bg-warning rounded-l-full" />
                <motion.div initial={{ width: 0 }} animate={{ width: `${nonCardPct}%` }} transition={{ duration: 0.8, delay: 0.2 }} className="h-full bg-primary/40 rounded-r-full" />
              </div>
              <div className="flex justify-between text-[9px] text-muted-foreground">
                <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-warning" />Cartão {cardPct}%</div>
                <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-primary/40" />Outros {nonCardPct}%</div>
              </div>
            </div>
      ...
      {/* ── Análise de Categorias (diagnostic style) ── */}
      <Section icon={<BarChart3 className="w-3.5 h-3.5" />} title="Análise de Categorias" delay={0.26}
      ...
      >
        <div className="space-y-3">
            ...
            return (
              <div key={cat.name} className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[12px] font-bold text-foreground tabular-nums">{fmt(cat.amount)}</span>
                    <span className="text-[10px] text-muted-foreground tabular-nums">{catPctVal}%</span>
                ...
                <div className="h-1.5 rounded-full bg-border/10 overflow-hidden">
                  <motion.div initial={{ width: 0 }} animate={{ width: `${Math.min(catPctVal, 100)}%` }} transition={{ duration: 0.6 }}
          ...
          {/* Diagnostics — the real analysis */}
          {catDiagnostics.length > 0 && (
            <div className="space-y-1.5 pt-1">
              <p className="text-[10px] font-semibold tracking-[0.5px] text-muted-foreground/70 uppercase mb-1">Diagnóstico</p>
              {catDiagnostics.map((diag, i) => (
                <AnalysisPill key={i} icon={
                  diag.variant === "success" ? <TrendingDown className="w-3 h-3" /> :
                  diag.variant === "danger" ? <AlertTriangle className="w-3 h-3" /> :
