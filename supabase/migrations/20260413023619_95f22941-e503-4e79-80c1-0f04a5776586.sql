...
-- =============================================
-- STEP 2: Create all triggers
-- =============================================

-- 1. Account balance recalculation on transaction changes
CREATE TRIGGER trg_update_account_balance_on_transaction
AFTER INSERT OR UPDATE OR DELETE ON public.transactions
FOR EACH ROW
EXECUTE FUNCTION public.update_account_balance();

-- 2. Account balance recalculation on invoice payment changes
CREATE TRIGGER trg_update_account_balance_on_invoice_payment
AFTER INSERT OR UPDATE OR DELETE ON public.invoice_payments
FOR EACH ROW
EXECUTE FUNCTION public.update_account_balance();

-- 3. Credit card invoice items cleanup on DELETE (BEFORE trigger)
...
-- 10. Auto-update goal current amount
...
-- =============================================
-- STEP 3: RECONCILIATION — Recalculate ALL account balances from scratch
-- =============================================
...
-- Recalculate all credit card used limits
UPDATE public.credit_cards cc
SET used_limit = COALESCE((
  SELECT SUM(GREATEST(inv.total_amount - COALESCE(inv.paid_amount, 0), 0))
  FROM public.invoices inv
  WHERE inv.credit_card_id = cc.id
